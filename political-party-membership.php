 ...
 * Requires PHP: 8.0
 * Network: false
 * render_membership_form handle_membership_check render_admin_page add_member_page handle_admin_registration members_list_shortcode
...
final class PoliticalPartyMembership
{
    ...
    public function get_kenyan_counties(): array
    {
        return [
            "Baringo",
            "Bomet",
    ...
    public function handle_membership_check(): void
    {
        ...
        if (empty($kenyan_id)) {
            wp_send_json_error(
                __("Please enter your ID number.", PPM_TEXT_DOMAIN)
            );
        }
        ...
        if (!preg_match('/^[0-9]{7,8}$/', $kenyan_id)) {
            wp_send_json_error(
                __(
                    "Please enter a valid Kenyan ID number (7-8 digits only).",
                    PPM_TEXT_DOMAIN
                )
            );
        }
        ...
        if ($member) {
            ...
            $formatted_date = wp_date(
                "F j, Y",
                strtotime($member->registration_date)
            );
            ...
            wp_send_json_success([
                ...
                "county" => esc_html($member->county ?? "Not specified"),
            ...
            wp_send_json_error(
                __(
                    "No membership found for this ID number. You may need to register first.",
                    PPM_TEXT_DOMAIN
                )
            );
        }
    ...
    public function init(): void
    {
        ...
        add_action("wp_ajax_nopriv_register_member", [
            $this,
            "handle_registration",
        ]);
        add_action("wp_ajax_check_membership", [
            $this,
            "handle_membership_check",
        ]);
        add_action("wp_ajax_nopriv_check_membership", [
            $this,
            "handle_membership_check",
        ]);

        // Add shortcodes
        add_shortcode("party_membership_form", [
            $this,
            "membership_form_shortcode",
        ]);
        add_shortcode("party_members_list", [$this, "members_list_shortcode"]);
        add_shortcode("party_membership_stats", [
            $this,
            "membership_stats_shortcode",
        ]);
        add_shortcode("party_membership_checker", [
            $this,
            "membership_checker_shortcode",
        ]);
        ...
        if (
            ...
            (is_object($post) &&
                (has_shortcode($post->post_content, "party_membership_form") ||
                    ...
                    has_shortcode(
                        $post->post_content,
                        "party_membership_stats"
                    ) ||
                    has_shortcode(
                        $post->post_content,
                        "party_membership_checker"
                    )))
        ) {
            ...
            $ajax_script =
                ...
                "',
                    nonce: '" .
                wp_create_nonce("party_membership_nonce") .
                "'
                };
            ";
        ...
        <table class="ppm-table">
            <thead>
                <tr>
                    ...
                    <?php endif; ?>
                    <?php if ($atts["show_county"] === "yes"): ?>
                    <th><?php esc_html_e("County", PPM_TEXT_DOMAIN); ?></th>
                    <?php endif; ?>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <?php endif; ?>
                    <td><?php echo esc_html(
                        wp_date("M j, Y", strtotime($member->registration_date))
                    ); ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
    ...
    public function membership_checker_shortcode($atts): string
    {
        ...
        <div id="membership-checker" class="ppm-form-container ppm-checker">
            ...
            <p><?php esc_html_e(
                "Enter your Kenyan ID number to check your membership details.",
                PPM_TEXT_DOMAIN
            ); ?></p>
            
            <form id="membership-check-form" class="ppm-form">
                <div class="ppm-form-group">
                    <label for="check_kenyan_id"><?php esc_html_e(
                        "Kenyan ID Number",
                        PPM_TEXT_DOMAIN
                    ); ?></label>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function handleCheckSubmit(e) {
                ...
                if (!checkId) {
                    showMessage(messageDiv, 'error', '<?php esc_html_e(
                ...
                if (checkId.length < 7 || checkId.length > 8) {
                    showMessage(messageDiv, 'error', '<?php esc_html_e(
                        "Please enter a valid Kenyan ID number (7-8 digits)",
                        PPM_TEXT_DOMAIN
                    ); ?>');
                    return;
                }
                ...
                submitButton.textContent = '<?php esc_html_e(
                    "Checking...",
                    PPM_TEXT_DOMAIN
                ); ?>';
                    ...
                    if (data.success) {
                        ...
                        const memberInfo = `
                            ...
                            <p><strong><?php esc_html_e(
                                "Member Number:",
                                PPM_TEXT_DOMAIN
                            ); ?></strong> ${member.member_number}</p>
                            <p><strong><?php esc_html_e(
                                "Full Name:",
                                PPM_TEXT_DOMAIN
                            ); ?></strong> ${member.full_name}</p>
                            <p><strong><?php esc_html_e(
                                "Email:",
                                PPM_TEXT_DOMAIN
                            ); ?></strong> ${member.email}</p>
                            <p><strong><?php esc_html_e(
                                "Phone:",
                                PPM_TEXT_DOMAIN
                            ); ?></strong> ${member.phone}</p>
                            <p><strong><?php esc_html_e(
                                "Registration Date:",
                                PPM_TEXT_DOMAIN
                            ); ?></strong> ${member.registration_date}</p>
                            <p><strong><?php esc_html_e(
                                "Status:",
                                PPM_TEXT_DOMAIN
                            ); ?></strong> ${member.status}</p>
                        `;
                        showMessage(messageDiv, 'success', memberInfo);
                    } else {
                        showMessage(messageDiv, 'error', data.data || '<?php esc_html_e(
                            "No membership found for this ID number.",
                            PPM_TEXT_DOMAIN
                        ); ?>');
                    }
                })
                .catch(error => {
                    showMessage(messageDiv, 'error', '<?php esc_html_e(
                        "An error occurred while checking membership.",
                        PPM_TEXT_DOMAIN
                    ); ?>');
                })
                .finally(() => {
    ...
    <div id="party-membership-form" class="ppm-form-container">
        <form id="membership-registration-form" class="ppm-form">
            ...
            <div class="ppm-form-group">
                <label for="email"><?php esc_html_e(
                    "Email Address",
                    PPM_TEXT_DOMAIN
                ); ?> *</label>
            ...
            <div class="ppm-form-group">
                <label for="phone"><?php esc_html_e(
                    "Phone Number",
                    PPM_TEXT_DOMAIN
                ); ?> *</label>
            ...
            <div class="ppm-form-group">
                <label for="kenyan_id"><?php esc_html_e(
                    "Kenyan ID Number",
                    PPM_TEXT_DOMAIN
                ); ?> *</label>
                ...
                <small><?php esc_html_e(
                    "Enter your National ID number (numbers only)",
                    PPM_TEXT_DOMAIN
                ); ?></small>
            </div>
            
            <div class="ppm-form-group">
                <label for="county"><?php esc_html_e(
                    "County",
                    PPM_TEXT_DOMAIN
                ); ?> *</label>
                ...
                <small><?php esc_html_e(
                    "Select the county where you reside",
                    PPM_TEXT_DOMAIN
                ); ?></small>
            </div>
    ...
    <div id="membership-checker" class="ppm-form-container ppm-checker">
        ...
        <p><?php esc_html_e(
            "Already a member? Enter your Kenyan ID number to check your membership details.",
            PPM_TEXT_DOMAIN
        ); ?></p>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function handleRegistrationSubmit(e) {
            ...
            submitButton.disabled = true;
            submitButton.innerHTML = '<span class="ppm-loading"></span><?php esc_html_e(
                "Registering...",
                PPM_TEXT_DOMAIN
            ); ?>';
            ...
            .then(data => {
                if (data.success) {
                    showMessage(messageDiv, 'success', 
                        '<?php esc_html_e(
                            "Registration successful! Your member number is:",
                            PPM_TEXT_DOMAIN
                        ); ?> <strong>' + 
                        data.data.member_number + '</strong><br><?php esc_html_e(
                            "Welcome to our political party!",
                            PPM_TEXT_DOMAIN
                        ); ?>');
            ...
            .finally(() => {
                submitButton.disabled = false;
                submitButton.innerHTML = '<?php esc_html_e(
                    "Register as Member",
                    PPM_TEXT_DOMAIN
                ); ?>';
            });
                ...
                if (data.success) {
                    ...
                    const memberInfo = `
                        ...
                        <p><strong><?php esc_html_e(
                            "County:",
                            PPM_TEXT_DOMAIN
                        ); ?></strong> ${member.county || 'Not specified'}</p>
                        <p><strong><?php esc_html_e(
                            "Registration Date:",
                            PPM_TEXT_DOMAIN
                        ); ?></strong> ${member.registration_date}</p>
        ...
        function validateRegistrationForm(formData, messageDiv) {
            ...
            const county = formData.get('county')?.trim();
            const termsAgreed = formData.get('terms_agreement');
            
            if (!fullName || !email || !phone || !kenyanId || !county) {
                showMessage(messageDiv, 'error', '<?php esc_html_e(
    ...
    public function handle_registration(): void
    {
        ...
        $nonce = sanitize_text_field(
            $_POST["nonce"] ?? ($_POST["membership_nonce"] ?? "")
        );
        if (!wp_verify_nonce($nonce, "party_membership_nonce")) {
            wp_send_json_error(__("Security check failed", PPM_TEXT_DOMAIN));
        }
        ...
        if (
            ...
            wp_send_json_error(
                __("Please fill in all required fields.", PPM_TEXT_DOMAIN)
            );
        }

        // Validate county selection
        $valid_counties = $this->get_kenyan_counties();
        if (!in_array($county, $valid_counties, true)) {
            wp_send_json_error(
                __("Please select a valid county.", PPM_TEXT_DOMAIN)
            );
        }
        ...
        if (!is_email($email)) {
            wp_send_json_error(
                __("Please enter a valid email address.", PPM_TEXT_DOMAIN)
            );
        }
        ...
        if (strlen($phone) < 10) {
            wp_send_json_error(
                __("Please enter a valid phone number.", PPM_TEXT_DOMAIN)
            );
        }
        ...
        if ($existing_email) {
            wp_send_json_error(
                __("This email address is already registered.", PPM_TEXT_DOMAIN)
            );
        }
        ...
        if ($existing_id) {
            wp_send_json_error(
                __(
                    "This ID number is already registered. Each ID number can only be used once.",
                    PPM_TEXT_DOMAIN
                )
            );
        }
        ...
        if ($result) {
            ...
            wp_send_json_error(
                __("Registration failed. Please try again.", PPM_TEXT_DOMAIN)
            );
        }
    }
    ...
    public function admin_page(): void
    {
        ...
        if (
            isset($_POST["action"], $_POST["party_members_nonce"]) &&
            wp_verify_nonce(
                $_POST["party_members_nonce"],
                "party_members_action"
            )
        ) {
            if (
            ...
            $members = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT * FROM {$this->table_name}" .
                        $where_clause .
                        " ORDER BY registration_date DESC LIMIT %d OFFSET %d",
    ...
    <div class="wrap">
        ...
        <div class="tablenav top">
                ...
                <form method="get" style="display: inline-block;">
                    ...
                    <?php if ($search): ?>
                        <a href="<?php echo esc_url(
                            admin_url("admin.php?page=party-members")
                        ); ?>" class="button">
            ...
            <div class="alignright">
                <strong><?php printf(
                    esc_html__("Total Members: %d", PPM_TEXT_DOMAIN),
                    $total_members
                ); ?></strong>
            </div>
        </div>
            ...
            <div class="tablenav top">
                <div class="alignleft actions bulkactions">
                    <select name="action" id="bulk-action-selector-top">
                        <option value="-1"><?php esc_html_e(
                            "Bulk Actions",
                            PPM_TEXT_DOMAIN
                        ); ?></option>
                        <option value="bulk_delete"><?php esc_html_e(
                            "Delete Selected",
                            PPM_TEXT_DOMAIN
                        ); ?></option>
                    </select>
                        ...
                        <?php foreach ($members as $member): ?>
                            <tr>
                                <th scope="row" class="check-column">
                                    <input type="checkbox" name="member_ids[]" value="<?php echo esc_attr(
                                        $member->id
                                    ); ?>">
                                </th>
                                ...
                                <td><a href="mailto:<?php echo esc_attr(
                                    $member->email
                                ); ?>"><?php echo esc_html(
                                    $member->email
                                ); ?></a></td>
                                <td><a href="tel:<?php echo esc_attr(
                                    $member->phone
                                ); ?>"><?php echo esc_html(
                                    $member->phone
                                ); ?></a></td>
                                ...
                                <td>
                                    <form method="post" style="display: inline;" 
                                          onsubmit="return confirmDelete('<?php echo esc_js(
                                              $member->full_name
                                          ); ?>');">
                                        <?php wp_nonce_field(
                                            "party_members_action",
    ...
    function confirmDelete(memberName) {
        return confirm('<?php esc_html_e(
            "Are you sure you want to delete the member",
            PPM_TEXT_DOMAIN
        ); ?> "' + memberName + '"? <?php esc_html_e(
    ...
    function handleBulkAction() {
        ...
        if (action === 'bulk_delete') {
            ...
            if (checkedBoxes.length === 0) {
                alert('<?php esc_html_e(
                    "Please select at least one member to delete.",
                    PPM_TEXT_DOMAIN
                ); ?>');
                return false;
            }
            return confirm('<?php esc_html_e(
                ...
                PPM_TEXT_DOMAIN
            ); ?> ' + checkedBoxes.length + ' <?php esc_html_e(
    ...
    private function handle_delete_member(): void
    {
        ...
        if ($member_id <= 0) {
            $this->add_admin_notice(
                "error",
                __("Invalid member ID.", PPM_TEXT_DOMAIN)
            );
            return;
        }
        ...
        if (!$member) {
            $this->add_admin_notice(
                "error",
                __("Member not found.", PPM_TEXT_DOMAIN)
            );
            return;
        }
        ...
        if ($result !== false) {
            $this->add_admin_notice(
                ...
                sprintf(
                    __(
                        'Member "%s" has been successfully deleted.',
                        PPM_TEXT_DOMAIN
                    ),
    ...
    private function handle_bulk_delete(): void
    {
        ...
        if (empty($member_ids)) {
            $this->add_admin_notice(
                "error",
                __("No valid member IDs provided.", PPM_TEXT_DOMAIN)
            );
            return;
        }
        ...
        if ($deleted_count !== false) {
            $this->add_admin_notice(
                ...
                sprintf(
                    _n(
                        "%d member has been successfully deleted.",
                        "%d members have been successfully deleted.",
                        $deleted_count,
                        PPM_TEXT_DOMAIN
                    ),
                    $deleted_count
            ...
            $this->add_admin_notice(
                "error",
                __(
                    "Failed to delete members. Please try again.",
                    PPM_TEXT_DOMAIN
                )
            );
        }
    ...
    {
        if (
            isset($_POST["add_member"], $_POST["add_member_nonce"]) &&
    ...
    <div class="wrap">
        ...
        <form method="post" action="">
            ...
            <table class="form-table">
                <tr>
                    <th><label for="full_name"><?php esc_html_e(
                        "Full Name",
                        PPM_TEXT_DOMAIN
                    ); ?> *</label></th>
                    <td><input type="text" id="full_name" name="full_name" required class="regular-text" value="<?php echo esc_attr(
                        $_POST["full_name"] ?? ""
                    ); ?>"></td>
                </tr>
                <tr>
                    <th><label for="email"><?php esc_html_e(
                        "Email Address",
                        PPM_TEXT_DOMAIN
                    ); ?> *</label></th>
                    <td><input type="email" id="email" name="email" required class="regular-text" value="<?php echo esc_attr(
                        $_POST["email"] ?? ""
                    ); ?>"></td>
                </tr>
                <tr>
                    <th><label for="phone"><?php esc_html_e(
                        "Phone Number",
                        PPM_TEXT_DOMAIN
                    ); ?> *</label></th>
                    <td><input type="tel" id="phone" name="phone" required class="regular-text" value="<?php echo esc_attr(
                        $_POST["phone"] ?? ""
                    ); ?>"></td>
                </tr>
                <tr>
                    <th><label for="kenyan_id"><?php esc_html_e(
                        "Kenyan ID Number",
                        PPM_TEXT_DOMAIN
                    ); ?> *</label></th>
                    <td>
                        <input type="text" id="kenyan_id" name="kenyan_id" required class="regular-text" 
                               maxlength="8" pattern="[0-9]{7,8}" value="<?php echo esc_attr(
                                   $_POST["kenyan_id"] ?? ""
                               ); ?>">
                        <p class="description"><?php esc_html_e(
                            "Enter National ID number (7-8 digits only)",
                            PPM_TEXT_DOMAIN
                        ); ?></p>
                    </td>
                </tr>
                <tr>
                    <th><label for="county"><?php esc_html_e(
                        "County",
                        PPM_TEXT_DOMAIN
                    ); ?> *</label></th>
                    <td>
                        <select id="county" name="county" required class="regular-text">
                            <option value=""><?php esc_html_e(
                                "Select county",
                                PPM_TEXT_DOMAIN
                            ); ?></option>
                        ...
                        <p class="description"><?php esc_html_e(
                            "Select the county where the member resides",
                            PPM_TEXT_DOMAIN
                        ); ?></p>
                    </td>
                </tr>
    ...
    private function handle_admin_registration(): void
    {
        ...
        if ($existing_id) {
            $this->add_admin_notice(
                "error",
                __("This ID number is already registered.", PPM_TEXT_DOMAIN)
            );
            return;
        }
        ...
        if ($result) {
            $this->add_admin_notice(
                ...
                sprintf(
                    __(
                        "Member successfully added with number: %s",
                        PPM_TEXT_DOMAIN
                    ),
            ...
            $this->add_admin_notice(
                "error",
                __("Failed to add member. Please try again.", PPM_TEXT_DOMAIN)
            );
        }
    }
}
...
register_uninstall_hook(__FILE__, [
    "PoliticalPartyMembershipUninstaller",
    "uninstall",
]);
